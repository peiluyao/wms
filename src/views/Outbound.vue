      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left">
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-for="order in outboundOrders" :key="order.id" class="hover:bg-slate-50/50 transition-colors">
              <td class="px-6 py-4 font-bold text-slate-700">{{ order.order_no }}</td>
              <td class="px-6 py-4">
                <span class="px-3 py-1 bg-blue-50 text-blue-600 rounded-full text-xs font-bold">已完成</span>
              </td>
            ...
            <tr v-if="outboundOrders.length === 0">
              <td colspan="5" class="px-6 py-12 text-center text-slate-400 font-medium">暂无出库记录</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div v-else class="bg-white rounded-3xl border border-slate-100 shadow-sm p-8 max-w-4xl mx-auto">
      <form @submit.prevent="handleSubmit" class="space-y-8">
        <div class="grid grid-cols-2 gap-8">
          <div class="space-y-2">
            <label class="text-xs font-bold text-slate-400 uppercase tracking-wider">出库单号</label>
            <input v-model="newOrder.order_no" type="text" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all font-medium" placeholder="CK-YYYYMMDD-XXX" required />
          </div>
          <div class="space-y-2">
        ...
        <div class="space-y-4">
          ...
          <div v-for="(item, index) in newOrder.items" :key="index" class="grid grid-cols-12 gap-4 items-end bg-slate-50 p-4 rounded-2xl border border-slate-100">
            ...
            <div class="col-span-4 space-y-2">
              <label class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">源库位</label>
            ...
            <div class="col-span-3 space-y-2">
              <label class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">出库数量</label>
        ...
        <div class="flex justify-end space-x-4 pt-6">
          ...
          <button type="submit" class="px-12 py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 shadow-lg shadow-blue-200 transition-all">提交出库</button>
        </div>
      </form>
    </div>
  </div>
...
const newOrder = reactive({
  order_no: `CK-${new Date().toISOString().slice(0, 10).replace(/-/g, '')}-${Math.floor(Math.random() * 1000).toString().padStart(3, '0')}`,
  remark: '',
  items: [{ material_id: null, location_id: null, qty: 1 }]
});

const outboundOrders = computed(() => orders.value.filter(o => o.type === 'OUTBOUND'));
...
const handleSubmit = async () => {
  try {
    await axios.post('/api/orders', { ...newOrder, type: 'OUTBOUND' });
